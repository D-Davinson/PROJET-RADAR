...
def get_scholar_profile_url(author_name):
    ...
    try:
        ...
        author_cache[author_name] = profile_url
        return profile_url
    except StopIteration:
        return None

# ✅ Scraping du h-index depuis Google Scholar (cache + timeout)
def get_h_index_from_scholar(profile_url):
    ...
    time.sleep(3)  # Pause pour éviter le bannissement
    ...
    if response.status_code == 200:
        ...
        author_cache[profile_url] = h_index
        return h_index
    return 0

# ✅ Recherche groupée des auteurs sur Google (évite plusieurs requêtes Scholar)
...
# ✅ Fonction optimisée de recherche d’un auteur et h-index
def search_scholar_with_h_index(query, max_articles=5):
    ...
    if query in author_cache:
        return author_cache[query]
    try:
        ...
        publications = author.get("publications", [])[:max_articles]
        articles = [
            {
        ...
        result = {"name": author_name, "affiliation": affiliation, "h_index": h_index, "articles": articles}
        author_cache[query] = result
        return result
    except StopIteration:
...
# Page Radar
    ...
    # ✅ Interface Streamlit pour la recherche par thème
        ...
        if search_theme:
            try:
                ...
                for _ in range(5):  # ✅ Moins de requêtes
                    try:
                ...
                # ✅ Recherche groupée sur Google pour les auteurs
